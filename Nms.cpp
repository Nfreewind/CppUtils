#include "Nms.h"
bool Nms::judge(const pair<cv::Rect, float> a, const pair<cv::Rect, float> b)
{
	return a.second > b.second;
}

void Nms::NmsMax(vector<cv::Rect>& rects, vector<float>& scores, int model, float overlap)
{
...
float Nms::IoU(cv::Rect r1, cv::Rect r2, int model)
{
	int h = min(r1.y + r1.height,r2.y + r2.height) - max(r1.y, r2.y);
	int w = min(r1.x + r1.width,r2.x + r2.width) - max(r1.x, r2.x);
...
vector<int> Nms::Nms(vector<Rect>& rects, vector<float>& scores, vector<int>& Srect, float overlap, Mat Img)
{
...
int Nms::Partation(Mat predicate, vector<int>& label) 
{
...
int Nms::Find(vector<int>& parent, int x) 
{
	int root = parent[x];
	if (root != x)
...
vector<float> Nms::Logistic(vector<float> scores, vector<int> index) 
{
	vector<float> Y;
